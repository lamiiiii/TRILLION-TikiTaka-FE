...
export default function RegisterRequestForm({onClose, name, firstCategoryId, secondCategoryId, token}: RegisterRequestFormProps) {
  ...
  const handleSubmit = async () => {
    if (!mustDescription.trim() || !description.trim()) {
      alert('필수 요청 사항과 양식 내용을 입력해주세요.');
      return;
    }

    try {
      setLoading(true);
      await createTicketForm(token, firstCategoryId, secondCategoryId, { mustDescription, description });
      alert('요청 양식이 성공적으로 생성되었습니다.');
      handleClose();
    } catch (error) {
      alert('요청 양식 생성에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <AnimatePresence>
      {!isClosing && ( // 닫힘 상태가 아닐 때만
        <motion.div
          ...
          className="fixed top-0 right-0 w-[750px] h-full bg-white shadow-lg z-50 p-6 flex flex-col"
        >
          ...
          <div className="w-[700px] h-[530px] bg-gray-18 mt-6 px-4 mx-auto shadow-[0px_1px_3px_1px_rgba(0,0,0,0.15)]">
            <div className="mt-4">
